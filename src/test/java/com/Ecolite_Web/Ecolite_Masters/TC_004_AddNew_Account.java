...
public class TC_004_AddNew_Account extends TestBase{
	...
	@Test
	public void AddNewAcc_Test() throws InterruptedException{
		...
		mp.SelectMasterFromMasterPage("account-master");
		
			String NewAccNameCreated = mp.AddNewAccountName().toString();
			NewAccNameCreated = NewAccNameCreated.toUpperCase();
			String AccNameFound = mp.SearchAccName().toString();
			AccNameFound = AccNameFound.toUpperCase();
			
			try{
			Assert.assertEquals(AccNameFound,NewAccNameCreated);
		}
		catch(Exception e){
			e.printStackTrace();
		}
